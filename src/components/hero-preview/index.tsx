...
export default function HeroPreview({ className }: Props) {
  ...
  return (
    <div className={`hidden sm:flex w-full flex-1 min-h-0${className ? ` ${className}` : ''}`}>
      ...
      {/* Preview: fills remaining space, clips its own overflow */}
      <div className="flex-1 min-h-0 relative overflow-hidden border-black/80 dark:border-white bg-linear-to-br/oklch from-zinc-300 to-zinc-100 dark:bg-linear-to-br/oklch dark:from-zinc-500 dark:to-slate-900 rounded-[10px]">
        <AnimatePresence>
          <motion.div
            key={previewSrc}
            initial={{ opacity: 0 }}
