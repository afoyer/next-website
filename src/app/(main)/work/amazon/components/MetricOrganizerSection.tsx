...
import { ArrowUpDown, Atom, Code, Database, FunctionSquare, Server } from "lucide-react";
	...
	return (
		<>
			...
			<div
				ref={whatIsItRef}
				id="step-1"
				className="flex flex-wrap gap-4 items-stretch justify-center sm:flex-col sm:min-h-screen sm:justify-center"
			>
				<motion.div className="grow shrink basis-xs flex flex-col gap-4" {...fadeUp(0.1)}>
					<CloudscapeContainer header="What is it?">
						<p className="mb-4">
							...
							metrics based on a configuration, allowing non-technical users to generate their own
							pages while we handle the backend fetching and resolving of a defined metric on a data
							center level scale.
						</p>
						<div className="mb-4">
					...
					<CloudscapeContainer>
						<p>
							This solution is built on top a grid layout system that extends further than just
							metrics. This allows us to place graphics as well as more logical layout containers
							and tabs to keep track of a data center&apos;s status.
						</p>
						<p className="mt-4">
							This layout system allows for dynamic loading through simple rendering techniques
							thanks to React and the templating standardization we set up for every configuration,
							allowing us to easily render many different equipments using only one configuration
							template.{" "}
							<b>
								It ultimately empowers us to render and populate{" "}
								<span className="underline font-bold">hundreds of thousands of metrics</span> over
								50 different pages, each having up to 30 different devices, in more than 50
								different data centers worldwide (and growing!).
							</b>
						</p>
					</CloudscapeContainer>
				</motion.div>
			...
			<div
				...
				id="step-2"
				className="flex flex-col gap-4 sm:min-h-screen sm:justify-center"
			>
				<CloudscapeContainer header="Extensibility">
					...
					<p className="mb-4">
						This led to abstracting the definition of a point to how it is fetched, allowing us to
						key on what kind of fetching a point would need beforehand then making a large GraphQL
						query to fetch all the metrics with their specific needs.
					</p>
					{/* Inline images — visible on mobile only */}
					<div
						className={`flex justify-center flex-wrap items-center gap-8 ${hideImages ? "sm:hidden" : ""}`}
					>
				...
				<CloudscapeContainer header="Configuration schema">
					<p className="text-gray-400 italic">
						Placeholder — describe how the configuration schema is structured and how developers
						define new metric types.
					</p>
				</CloudscapeContainer>

				<CloudscapeContainer header="Adding a new metric type">
					<p className="text-gray-400 italic">
						Placeholder — walk through the steps a developer follows to register a new metric, from
						schema definition to GraphQL resolver. Another section that we had to expand to enhance
						user experience (UX) was the ability to edit these configurations without the need to
						understand how the configuration files functioned. This lead to a large prototyping
						phase to understand flows and how to implement them. Another section that we had to
						expand to enhance user experience (UX) was the ability to edit these configurations
						without the need to understand how the configuration files functioned. This lead to a
						large prototyping phase to understand flows and how to implement them. Another section
						that we had to expand to enhance user experience (UX) was the ability to edit these
						configurations without the need to understand how the configuration files functioned.
						This lead to a large prototyping phase to understand flows and how to implement them.
					</p>
				</CloudscapeContainer>
			</div>
			...
			>
				<CloudscapeContainer header="Tooling">
					...
					<div
						className={`flex justify-center flex-wrap items-center gap-4 ${hideImages ? "sm:hidden" : ""}`}
					>
						...
						<p className="text-center text-sm grow shrink basis-50 min-w-50">
							Prototype of the MetricOrganizer configuration editor. This added functionality to
							auto suggest metrics, guardrailing potential errors, and making the editing process
							smoother.
						</p>
					</div>
				</CloudscapeContainer>
				...
				<CloudscapeContainer header="Auto-suggest & guardrailing">
					<p className="text-gray-400 italic">
						Placeholder — explain how the editor surfaces metric suggestions and prevents invalid
						configurations at authoring time.
					</p>
				</CloudscapeContainer>

				<CloudscapeContainer header="Prototyping process">
					<p className="text-gray-400 italic">
						Placeholder — describe the design and prototyping phases that informed the final editor
						UX, including user testing with non-technical operators.
					</p>
				</CloudscapeContainer>
			</div>
		</>
