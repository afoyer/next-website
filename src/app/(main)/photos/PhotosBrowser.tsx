...
type Photo = NonNullable<
	NonNullable<Awaited<ReturnType<typeof client.queries.listPhotos>>["data"]>[number]
>;
...
function originTransform(rect: DOMRect, photo: Photo) {
	...
	const ar =
		photo.widthL && photo.heightL ? photo.widthL / photo.heightL : rect.width / rect.height;
	let tw = maxW;
	let th = tw / ar;
	if (th > maxH) {
		th = maxH;
...
export default function PhotosBrowser() {
	...
	const { data: photos = [], isFetching: photosLoading } = useQuery({
		...
		queryFn: () =>
			client.queries.listPhotos({ albumId: selectedId }).then(({ data }) => data ?? []),
		enabled: !!selectedId,
	});
							...
							{photos.map((photo, i) => {
								...
								return (
									<motion.div
										...
										onClick={(e) =>
											setLightbox({ photo, rect: e.currentTarget.getBoundingClientRect() })
										}
									>
										<img
											src={src}
